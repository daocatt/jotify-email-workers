...
### 1. 邮件托管和转发
* **多租户机制**：用户可自主登录系统，并管理属于自己的域名与收信规则。
...
* **Cloudflare Email Routing**：
  * **一句话介绍**：在 Cloudflare 中配置自定义域名即可收取电子邮件。
...
| `SUPERADMIN_EMAIL` | 是 | `jotify@zwq.me` | 默认初始超级管理员的登录邮箱（数据库为空时会自动初始化）。 |
...
你的应用（如博客、日记平台 Jotify）需要支持“通过发送邮件直接写日记/发布文章”的功能。
